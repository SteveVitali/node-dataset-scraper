...
class Scraper {
  constructor(params) {
    ...
    this.logs_path = params.logs_path ||
                    `${this.outputDir}/${(new Date()).toJSON()}.log`;
    this.outputPath = `${this.outputDir}/output.json`;
    this.failurePath= `${this.outputDir}/failures.json`;
    ...
    this.input_path = params.input_path || (throw 'Invalid input path');
    this.inputIds = JSON.parse(fs.readFilesync(this.input_path));

    this.currentIndex = 0;

    this.makeUrl = params.makeUrl || (throw 'Must provide URL generator');
    ...
    return new Promise((resolve, reject) => {
      ...
      async.eachLimit(this.inputIds, this.nThreads, (docId, done) => {
        ...
        if (docId in this.scrapedMetadataMap) {
          ...
          if (docId in this.failedScrapesMap) markFailureAsCorrected(docId)
        ...
        this.doGET(docId, (err, html) => {
          ...
          const secs = ((new Date()) - START_TIME) / 1000;
  ...
  doGET(id, done) {
    ...
    request({
      uri: url,
      jar: this.cookieString ? makeCookieJar(url) : undefined,
      method: 'GET',
